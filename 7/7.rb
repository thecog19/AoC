class TreeGenerator
	Node = Struct.new(:name, :children, :weight, :netWeight, :parent)
	...
	def generateTree(instructions)
		instructions.each do |instruction|
			command = instruction.split("-> ")
			weight = instruction.split("(")[1].split(")")[0].to_i
			nodeName = command[0].split(" ")[0]
			if(@hashOfNodes[nodeName])
				@hashOfNodes[nodeName].weight = weight
			else
				@hashOfNodes[nodeName] = Node.new(nodeName, [], weight)
			end
			unless(command[1])
				next 
			end
			command[1].split(", ").each do |node|
				if(@hashOfNodes[node])
					@hashOfNodes[nodeName].children.push(@hashOfNodes[node])
				else
					@hashOfNodes[node] = Node.new(node, [])
					@hashOfNodes[nodeName].children.push(@hashOfNodes[node])
				end

			end
		end
		root = findRoot(@hashOfNodes)
		generateNetWeightsForTree(root)
		findUnbalanced(root)

		# print @hashOfNodes["bpvhwhh"]
		# print @hashOfNodes["kfdxxb"]
	end

	def findRoot(nodeHash)
		keys = nodeHash.keys
		#the basic idea is we're going to go, and eliminate any keys where the children aren't in our hash
		#repeat this until keys has a length of 1, that's our root
		while keys.length > 1
			nodeHashClone = nodeHash.clone
			keys.each do |key|
				if(nodeHashClone[key].children.empty?)
					nodeHash.delete(key)
					next
				end

				unless(nodeHashClone[key].children.any?{|child| nodeHashClone[child.name]})
					nodeHash.delete(key)
				end
			end
			keys = nodeHash.keys
		end
		return nodeHash[keys[0]]
	end

	def generateNetWeightsForTree(root)
		if(root.children.empty?)
			root.netWeight = root.weight 
			return root.netWeight
		end
		root.netWeight = root.weight
		root.children.each do |child|
			root.netWeight += generateNetWeightsForTree(child)
		end
		return root.netWeight
	end

	def findUnbalanced(root)
		counts = Hash.new(0)
		root.children.each { |child| counts[child.netWeight] += 1 }
		root.children.each {|child| child.parent = root}
		key = counts.select { |v, count| count == 1 }.keys[0] # [1, 3]
		child = root.children.select{|element| element.netWeight == key}[0]
		if(child)
			findUnbalanced(child)
		else
			root.parent.children.each do |child|
				puts "child weight #{child.weight} child netWeight #{child.netWeight}"
			end
		end
	end
end
...
TreeGenerator.new("navfz (187) -> jviwcde, wfwor, vpfabxa
adtfnx (74)
endsg (33)
