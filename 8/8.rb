class Interpreter
	...
	INSTRUCTION = {
		"dec" => lambda {|reg, val| return reg - val},
		"inc" => lambda {|reg, val| return reg + val},
	}
	...
	def parseInstructions(insts)
		insts.each do |inst|
			instArray = inst.split(" ")

			mainRegister = instArray[0]
			direction = instArray[1]
			value = instArray[2].to_i

			compareReg = instArray[4]
			comparator = instArray[5]
			compareVal = instArray[6].to_i
			if(evaluateInstruction(compareReg, comparator, compareVal))
				updateHighest(runInstruction(mainRegister, direction, value))
			end
		end
	end

	def getMax
		@register.max_by{|k,v| v}
	end
	...
	def runInstruction(regKey, instruction, value)
		regValue = getRegister(regKey)
		newVal = INSTRUCTION[instruction].call(regValue, value) 
		setRegister(regKey, newVal) 
		return getRegister(regKey)
	end

	def setRegister(key, newVal)
		@register[key] = newVal
	end

	def evaluateInstruction(regKey, comparator, value)
		regValue = getRegister(regKey)
		COMPARATORS[comparator].call(regValue, value)
	end
...
puts Interpreter.new("sd dec 441 if k != 0
lp dec 419 if mxn >= 7
w inc -592 if icg >= -9
a dec -29 if q <= 9
kt dec 486 if ms != 8
...
cl dec -167 if gi == 1209").getHighest()
