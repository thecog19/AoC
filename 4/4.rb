def hasDuplicates?(arr)
	return arr.group_by{ |element| element }.select { |key, value| value.size > 1 }.keys.length > 0
	
end

def hasAnagramDuplicates?(arr)
	# puts(arr.group_by{ |element| element.split("").sort().join("")}.select { |key, value| value.size > 1 }.keys.length)
	return arr.group_by{ |element| element.split("").sort().join("")}.select { |key, value| value.size > 1 }.keys.length > 0
end
...
def arrayCounter(arr)
	...
	arr.each do |str|
		subArray = str.split(" ")
		unless(hasDuplicates?(subArray))
			counter += 1
		end
		unless(hasAnagramDuplicates?(subArray))
			anaCounter += 1
		end
	end
	puts "the final count of duplicates is #{counter}"
	puts "the final count of anagram duplicates is #{anaCounter}"
end
